# Hi, I'm Jameel Shaikh 👋

## Quantitative Finance & Risk Analytics

Graduate student focused on quantitative finance, risk analytics, and data-driven decision making. Building projects involving exploratory data analysis, financial modeling, visualization, and operational analytics.

---

## Technical Skills

- Python
- pandas
- matplotlib
- seaborn
- Excel
- Exploratory Data Analysis (EDA)
- Data Visualization
- Statistical Analysis
- Risk Analytics
- Financial Analytics

---

## Featured Projects

### Bike Rental Demand Analysis
Python-based exploratory data analysis project examining commuter demand patterns, weather impacts, and operational strategy insights using real-world rental data.

🔗 Repository:
https://github.com/Jams411/bike-rental-demand-analysis

---

### Squid Game Text Adventure
Interactive Python text adventure game using branching logic, user input, and storytelling mechanics inspired by Squid Game.

🔗 Repository:
https://github.com/Jams411/squid-game-text-adventure

---

## Current Focus
- Time Series Analysis
- Financial Risk Modeling
- Quantitative Finance
- Machine Learning
- Portfolio Analytics

---

## Connect With Me
- LinkedIn: https://www.linkedin.com/in/jams411/
